...
void DiffusionSammy::EvaluateDiffusion(void)
{
   ...
   if (_spdata.Uvec.Norm() > 3.0) {
      Kappa[1] = ((3 * _vel[0] * _vel[0] * unit_velocity_fluid * unit_velocity_fluid * GSL_CONST_CGSM_MASS_PROTON * GSL_CONST_CGSM_MASS_PROTON)
               / (20 * SPC_CONST_CGSM_ELECTRON_CHARGE * SPC_CONST_CGSM_ELECTRON_CHARGE * sin(3 * M_PI / 5))
               * (1 + (72/7)*pow(
                  (_spdata.region[l_perp_index] * unit_length_fluid * SPC_CONST_CGSM_ELECTRON_CHARGE * _spdata.Bmag * unit_magnetic_fluid)/(GSL_CONST_CGSM_MASS_PROTON * _vel[0] * unit_velocity_fluid), 5/3
               ))
               / (_spdata.region[l_perp_index] * unit_length_fluid * _spdata.region[del_b_index]*_spdata.region[del_b_index]*unit_magnetic_fluid*unit_magnetic_fluid));
   }
   else {
      Kappa[1] = (
         (1 / 9.7705) * pow(GSL_CONST_CGSM_SPEED_OF_LIGHT * GSL_CONST_CGSM_MASS_PROTON * GSL_CONST_CGSM_MASS_PROTON / SPC_CONST_CGSM_ELECTRON_CHARGE, 1/3)
         * pow(RelFactor1(_mom[0]), 1/3) * _vel[0] * _vel[0] * unit_velocity_fluid * unit_velocity_fluid
         * pow(_spdata.region[l_perp_index] * unit_length_fluid, 2/3)
         * 0.144
         / (0.3 * unit_magnetic_fluid)
      );
   };
   Kappa[0] = kap_rat * Kappa[1];
};
